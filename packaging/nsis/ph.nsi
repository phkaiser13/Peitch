...
*
* File: ph.nsi (Professional Enhanced Version)
*
* This is a professional-grade NSIS installer for the ph application with advanced
...
!define PRODUCT_VERSION "@ph_VERSION@"
...
!define PRODUCT_SUPPORT_URL "https://github.com/phkaiser13/peitch/issues"
!define PRODUCT_UPDATE_URL "https://github.com/phkaiser13/peitch/releases"
!define PRODUCT_HELP_URL "https://github.com/phkaiser13/peitch/wiki"
...
OutFile "ph-${PRODUCT_VERSION}-installer.exe"
...
!define MUI_ICON "ph.ico"
!define MUI_UNICON "ph.ico"
...
;================================
!define MUI_FINISHPAGE_RUN "$INSTDIR\bin\ph.exe"
...
Function InitializeLogging
  ...
  StrCpy $LogFile "${SYSTEM_LOGS_DIR}\ph_installer_$2$1$0_$4$5$6.log"
  
  ; Initialize log file
  FileOpen $MainLogHandle "$LogFile" w
  ${If} $MainLogHandle != ""
    FileWrite $MainLogHandle "=== ph Professional Installer Log ===$\r$\n"
...
Function InstallService
  Push "Installing ph service..."
  Call WriteLog
  
  ; Check if service already exists
  SimpleSC::ExistsService "ph"
  Pop $0
  ${If} $0 == 0
    Push "ph service already exists, stopping it first"
    Call WriteLog
    SimpleSC::StopService "ph" 1 30
    SimpleSC::RemoveService "ph"
  ${EndIf}
  
  ; Install new service
  SimpleSC::InstallService "ph" "ph Service" "16" "2" "$INSTDIR\bin\ph-service.exe" "" "" ""
  Pop $0
  
  ${If} $0 == 0
    Push "ph service installed successfully"
    Call WriteLog
    
    ; Set service description
    SimpleSC::SetServiceDescription "ph" "ph workflow management service with HashiCorp Vault integration"
    
    ; Start service
    SimpleSC::StartService "ph" "" 30
    Pop $1
    ${If} $1 == 0
      Push "ph service started successfully"
      Call WriteLog
      StrCpy $ServiceInstalled "1"
    ${Else}
      Push "Failed to start ph service: $1"
      Call LogWarning
    ${EndIf}
  ${Else}
    Push "Failed to install ph service: $0"
    Call LogError
  ${EndIf}
FunctionEnd

Function UninstallService
  Push "Uninstalling ph service..."
  Call WriteLog
  ...
  ${If} $0 == 0
    ...
    ${If} $1 == 0
      Push "ph service uninstalled successfully"
      Call WriteLog
    ${Else}
      Push "Failed to remove ph service: $1"
      Call LogWarning
    ${EndIf}
  ${EndIf}
FunctionEnd
...
Function CreateFileAssociations
  ...
  ; Register .ph files
  !insertmacro WriteRegStrSafe HKCR ".ph" "" "ph.ConfigFile"
  !insertmacro WriteRegStrSafe HKCR ".ph" "Content Type" "application/json"
  !insertmacro WriteRegStrSafe HKCR "ph.ConfigFile" "" "ph Configuration File"
  !insertmacro WriteRegStrSafe HKCR "ph.ConfigFile\DefaultIcon" "" "$INSTDIR\bin\ph.exe,0"
  !insertmacro WriteRegStrSafe HKCR "ph.ConfigFile\shell\open\command" "" '"$INSTDIR\bin\ph.exe" "--config" "%1"'
  !insertmacro WriteRegStrSafe HKCR "ph.ConfigFile\shell\edit\command" "" '"notepad.exe" "%1"'
  
  ; Register URL protocol for ph://
  !insertmacro WriteRegStrSafe HKCR "ph" "" "URL:ph Protocol"
  !insertmacro WriteRegStrSafe HKCR "ph" "URL Protocol" ""
  !insertmacro WriteRegStrSafe HKCR "ph\DefaultIcon" "" "$INSTDIR\bin\ph.exe,1"
  !insertmacro WriteRegStrSafe HKCR "ph\shell\open\command" "" '"$INSTDIR\bin\ph.exe" "--handle-url" "%1"'
...
Function RemoveFileAssociations
  ...
  !insertmacro DeleteRegKeySafe HKCR ".ph"
  !insertmacro DeleteRegKeySafe HKCR "ph.ConfigFile"
  !insertmacro DeleteRegKeySafe HKCR "ph"
...
Function VerifyInstallation
  ...
  ; Check core files
  ${IfNot} ${FileExists} "$INSTDIR\bin\ph.exe"
    Push "ERROR: Core executable missing: ph.exe"
    Call LogError
    IntOp $0 $0 + 1
  ${EndIf}
  ...
  ; Verify executable
  GetDLLVersion "$INSTDIR\bin\ph.exe" $1 $2
  ${If} $1 == ""
  ${OrIf} $2 == ""
...
Function MaintenanceModePage
  ...
  ${NSD_CreateRadioButton} 20u 100u 80% 15u "Remove ph completely"
...
; Language Strings
;================================
LangString DESC_SecCore ${LANG_ENGLISH} "Core ph application and essential components. (Required)"
LangString DESC_SecCore ${LANG_PORTUGUESEBR} "Aplicação principal do ph e componentes essenciais. (Obrigatório)"
...
LangString DESC_SecPath ${LANG_ENGLISH} "Add ph to the system PATH for command-line access."
LangString DESC_SecPath ${LANG_PORTUGUESEBR} "Adicionar ph ao PATH do sistema para acesso via linha de comando."

LangString DESC_SecService ${LANG_ENGLISH} "Install ph as a Windows service for background operations."
LangString DESC_SecService ${LANG_PORTUGUESEBR} "Instalar ph como um serviço do Windows para operações em segundo plano."
...
Section "!ph Core" SecCore
  SectionIn RO
  
  SetOverwrite on
  SetOutPath "$INSTDIR"
  
  Push "Installing ph core components..."
  Call WriteLog
  ...
  ; Install core files
  File /oname=bin\ph.exe "ph.exe"
  File /oname=bin\ph-service.exe "ph-service.exe"
  File /oname=bin\ph-installer.exe "ph-installer.exe"
...
Function un.onInit
  ...
  StrCpy $LogFile "$InstallDir\logs\ph_uninstall.log"
  FileOpen $0 "$LogFile" w
  ${If} $0 != ""
    FileWrite $0 "=== ph Professional Uninstaller Log ===$\r$\n"
    ${GetTime} "" "L" $1 $2 $3 $4 $5 $6 $7
...
Function un.CustomUninstallPage
  ...
  ${NSD_CreateCheckBox} 10u 145u 80% 12u "Remove installed Git (if installed by ph)"
  Pop $0
  
  ${NSD_CreateCheckBox} 10u 165u 80% 12u "Remove installed Vault (if installed by ph)"
  Pop $0
...
Section "Uninstall"
  ...
  Pop $0
  ${If} $0 == 0
    DetailPrint "Stopping ph service..."
    SimpleSC::StopService "ph" 1 30
    DetailPrint "Removing ph service..."
    SimpleSC::RemoveService "ph"
  ${EndIf}
  ...
  ; Core files
  Delete "$INSTDIR\bin\ph.exe"
  Delete "$INSTDIR\bin\ph-service.exe"
  Delete "$INSTDIR\bin\ph-installer.exe"
  ...
  ; Remove Vault if installed by ph
  ReadRegStr $0 HKLM "${PRODUCT_INST_KEY}" "VaultInstalledByph"
  ${If} $0 == "1"
...
Function un.RemoveFileAssociations
  DeleteRegKey HKCR ".ph"
  DeleteRegKey HKCR "ph.ConfigFile"
  DeleteRegKey HKCR "ph"
...
Function un.onUninstSuccess
  ...
  MessageBox MB_OK|MB_ICONINFORMATION \
    "ph has been successfully removed from your system.$\r$\n$\r$\nThank you for using ph!"
FunctionEnd
...
VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "ph Professional Installer"
VIAddVersionKey /LANG=${LANG_ENGLISH} "FileVersion" "${PRODUCT_VERSION}.${BUILD_NUMBER}"
VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductVersion" "${PRODUCT_VERSION}"
VIAddVersionKey /LANG=${LANG_ENGLISH} "InternalName" "ph-installer"
VIAddVersionKey /LANG=${LANG_ENGLISH} "OriginalFilename" "ph-${PRODUCT_VERSION}-installer.exe"
...
!verbose 4
!echo "ph Professional Installer compiled successfully"
!echo "Version: ${PRODUCT_VERSION}"
!echo "Target: ph-${PRODUCT_VERSION}-installer.exe"
...
* and provides enterprise-grade functionality for the ph application.
*/
  Pop $3
  Pop $2
  Pop $1
